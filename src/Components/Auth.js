...
const Auth = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('https://craftopianewbackend-production.up.railway.app/api/v1/auth/login', formData);
  ...
  const handleRegister = async () => {
    try {
      const response = await axios.post('https://craftopianewbackend-production.up.railway.app/api/v1/auth/register', formData);
      const data = response.data;
